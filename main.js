...
/* harmony default export */ const js_handlerLongText = (handlerLongText);
;// ./src/js/converterTimestamp.js
...
/* harmony default export */ const js_converterTimestamp = (converterTimestamp);
  ...
  renderMessage() {
    ...
    stream$.subscribe(({
      ...
      messages.forEach(message => {
        ...
        const title = js_handlerLongText(message.subject);
        const date = js_converterTimestamp(message.received);
        messagesContainer.insertAdjacentHTML("afterBegin", Polling.markupMessage(id, email, title, date));
      });
    });
  }
  requestMessage() {
    const stream$ = interval(3000).pipe(switchMap(() => {
      return ajax(this.url).pipe(map(response => response), catchError(() => EMPTY));
    }));
    return stream$;
  }
  init() {
